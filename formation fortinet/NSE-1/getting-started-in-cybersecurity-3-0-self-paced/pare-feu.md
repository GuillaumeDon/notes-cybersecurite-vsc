...
FonctionDescriptionFiltrage de paquetsAnalyse les paquets et autorise/bloque selon les règlesInspection du traficExamine le contenu des communicationsContrôle d'accèsDétermine qui peut accéder à quelles ressourcesJournalisationEnregistre tous les événements pour auditTranslation d'adresses (NAT)Masque les adresses IP internesProtection contre les menacesDétecte et bloque les attaques connues
...
Le trafic arrive au pare-feu
Le pare-feu consulte les règles de haut en bas
Première règle correspondante = action appliquée
Si aucune règle ne correspond = blocage par défaut (deny all)
...
Politique de sécurité stricte : Bloquer par défaut, autoriser uniquement le nécessaire
Mise à jour régulière : Firmware et signatures de menaces
Surveillance continue : Analyser les logs quotidiennement
Segmentation réseau : Diviser le réseau en zones
Authentification forte : Pour l'administration du pare-feu
Sauvegarde de configuration : Régulière et testée
Documentation : Maintenir à jour les règles et justifications
...
Un pare-feu est la première ligne de défense d'un réseau
Les NGFW (comme FortiGate) offrent une protection multicouche
Les règles sont traitées de haut en bas (ordre important !)
Bloquer par défaut est la meilleure approche (whitelist)
Un pare-feu seul ne suffit pas : approche défense en profondeur
La configuration est aussi importante que le matériel
